...
const TestomonialCard = ({data}) => {
    ...
    const[currScreenSize,setCurrScreenSize] = useState();
    ...
    function handleScreenRes () {
        console.log(currScreenSize)
        setCurrScreenSize(window.screen.width);
    }

    return (
        <div onResize={handleScreenRes} className={`text-center p-7 shadow-[0px_1px_20px_0px_#00000052] rounded-lg`}>
            <div>
                <h4 onClick={handleTextExpand} className={`font-medium cursor-pointer md:cursor-default text-black mb-5`}>
                    {window.screen.width < 768 ? review.slice(0,textCollapse[0])+`${!textCollapse[1] && '...'}` : review}
                    </h4>
